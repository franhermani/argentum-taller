...
world_t ClientProtocol::receiveWorldUpdate() {
    ...
    for (i = 0; i < inventory_length; i ++) {
        uint8_t item_type;
        memcpy(&item_type, world_buffer.data() + pos, SIZE_8);
    ...
    memcpy(&num_players, world_buffer.data() + pos, SIZE_16);
    ...
    for (i = 0; i < w.num_players; i ++) {
        ...
        // Id
        uint16_t id;
        memcpy(&id, world_buffer.data() + pos, SIZE_16);
        player.id = ntohs(id);
        pos += SIZE_16;

        // Pos x en la matriz
        uint16_t pos_x;
        memcpy(&pos_x, world_buffer.data() + pos, SIZE_16);
        player.pos.x = ntohs(pos_x);
        pos += SIZE_16;

        // Pos y en la matriz
        uint16_t pos_y;
        memcpy(&pos_y, world_buffer.data() + pos, SIZE_16);
        player.pos.y = ntohs(pos_y);
        pos += SIZE_16;

        // Vida actual
        uint16_t actual_life;
        memcpy(&actual_life, world_buffer.data() + pos, SIZE_16);
        player.actual_life = ntohs(actual_life);
        pos += SIZE_16;

        // Vida maxima
        uint16_t max_life;
        memcpy(&max_life, world_buffer.data() + pos, SIZE_16);
        player.max_life = ntohs(max_life);
        pos += SIZE_16;

        // Nivel
        uint16_t level;
        memcpy(&level, world_buffer.data() + pos, SIZE_16);
        player.level = ntohs(level);
        pos += SIZE_16;

        // Enum type de la orientacion
        uint8_t orientation;
        memcpy(&orientation, world_buffer.data() + pos, SIZE_8);
        player.orientation = orientation;
        pos += SIZE_8;

        // Enum type del estado
        uint8_t state;
        memcpy(&state, world_buffer.data() + pos, SIZE_8);
        player.state = state;
        pos += SIZE_8;

        // Enum type de la raza
        uint8_t race_type;
        memcpy(&race_type, world_buffer.data() + pos, SIZE_8);
        player.race_type = race_type;
        pos += SIZE_8;

        // Enum type de la clase
        uint8_t class_type;
        memcpy(&class_type, world_buffer.data() + pos, SIZE_8);
        player.class_type = class_type;
        pos += SIZE_8;

        // Enum type del arma
        uint8_t weapon;
        memcpy(&weapon, world_buffer.data() + pos, SIZE_8);
        player.weapon = weapon;
        pos += SIZE_8;

        // Enum type de la armadura
        uint8_t armor;
        memcpy(&armor, world_buffer.data() + pos, SIZE_8);
        player.armor = armor;
        pos += SIZE_8;

        // Enum type del casco
        uint8_t helmet;
        memcpy(&helmet, world_buffer.data() + pos, SIZE_8);
        player.helmet = helmet;
        pos += SIZE_8;

        // Enum type del escudo
        uint8_t shield;
        memcpy(&shield, world_buffer.data() + pos, SIZE_8);
    ...
    memcpy(&num_creatures, world_buffer.data() + pos, SIZE_16);
    ...
    for (i = 0; i < w.num_creatures; i ++) {
        ...
        // Enum type del tipo
        uint8_t type;
        memcpy(&type, world_buffer.data() + pos, SIZE_8);
        creature.type = type;
        pos += SIZE_8;
    ...
    // Cantidad de Items
    uint16_t num_items;
    memcpy(&num_items, world_buffer.data() + pos, SIZE_16);
    ...
    // Cantidad de Oros
    uint16_t num_golds;
    memcpy(&num_golds, world_buffer.data() + pos, SIZE_16);
    ...
    for (i = 0; i < w.num_golds; i ++) {
        ...
        // Cantidad
        uint16_t quantity;
        memcpy(&quantity, world_buffer.data() + pos, SIZE_16);
    ...
    memcpy(&num_attacks, world_buffer.data() + pos, SIZE_16);
    ...
    for (i = 0; i < w.num_attacks; i ++) {
        ...
        uint8_t sound;
        memcpy(&sound, world_buffer.data() + pos, SIZE_8);
        attack.sound = sound;
        pos += SIZE_8;

        // 1 si esta colisionando, 0 si no
        uint8_t is_colliding;
        memcpy(&is_colliding, world_buffer.data() + pos, SIZE_8);
