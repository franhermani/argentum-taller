...
BuyItemCommandDTO::BuyItemCommandDTO(const uint8_t item_type,
        const uint16_t npc_pos_x, const uint16_t npc_pos_y) :
        itemType(item_type), NPCPosX(npc_pos_x),
        NPCPosY(npc_pos_y) {}

BuyItemCommandDTO::~BuyItemCommandDTO() = default;

const std::vector<char> BuyItemCommandDTO::serialize() const {
